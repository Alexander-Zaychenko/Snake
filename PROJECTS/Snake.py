...
pygame.display.set_caption("Rasdsa")
...
snake_speed = int(input('Speed: '))

# snake tails
snake_tails = []

snake_pos["x_change"] = -snake_speed
for i in range(int(input("Start lenght of snake: "))):
...
food_pos = {
    ...
    "x2": random.randint(0, 640) / 10,
    "y2": random.randint(0,480) / 10,
}
print(food_pos['x'])
print(food_pos['x2'])
food_size = (10, 10)
...
while not game_end:
    ...
    # # second food)
    # pygame.draw.rect(display, colors["apple"], [
    #     random.randint(0, 640) / 10,
    #     random.randint(0, 480) / 10,
    #     food_size[0],
    #     food_size[1]])

    # detect collision with food
    if (snake_pos["x"] == food_pos["x"]
            and snake_pos["y"] == food_pos["y"]): # or (snake_pos["x"] == food_pos["x2"] and snake_pos["y"] == food_pos["y2"])
        food_eaten += 1
        print(f'{food_eaten = }')
